                ...
                <BlurFade key={index} delay={0.2 + index * 0.15} duration={0.5} className="flex">
                  <div
                    className={`relative flex flex-col items-center text-center w-full sm:w-48 md:w-56 lg:w-64 p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl border transition-all duration-300 ${
                      isFocused
                        ? 'border-amber-400/60 bg-gradient-to-br from-amber-950/40 to-slate-900/60 sm:scale-105'
                        : 'border-slate-700/40 bg-slate-900/40 hover:border-amber-500/40 hover:bg-slate-900/60'
                    }`}
                  >
                    {/* Focus indicator */}
                    {isFocused && !slide.allActive && (
                      <div className="absolute -top-2 sm:-top-3 left-1/2 -translate-x-1/2 px-2 py-0.5 sm:px-3 sm:py-1 bg-amber-500 text-slate-900 text-[10px] sm:text-xs font-bold rounded-full">
                        UP NEXT
                      </div>
                    )}

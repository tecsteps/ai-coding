...
export function G03_Balleyguier({ index, total }: Props) {
  return (
    <AlphalistFrame
      slideNumber={index + 1}
      totalSlides={total}
      eyebrow="Day 1 · 14:30 · Fireside Chat"
      title={
        <>
          Fireside Chat:{' '}
          <span className="text-emerald-600">André Balleyguier (Anthropic)</span> + Q&amp;A
        </>
      }
    >
        ...
        <BlurFade delay={0.15} duration={0.6}>
          <aside className="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-[0_24px_70px_-46px_rgba(15,23,42,0.65)]">
            <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-slate-100">
              <Image
                src="/alphalist/guests/andre-balleyguier.jpg"
                alt="André Balleyguier"
                fill
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 px-4 py-4">
              <p className="text-2xl font-semibold text-slate-900">André Balleyguier</p>
              <p className="mt-1 text-base text-slate-500">Applied AI Leader, EMEA</p>
              <p className="mt-1 text-base font-semibold text-emerald-600">Anthropic</p>
              <a
                href="https://www.linkedin.com/in/aballeyguier"
                target="_blank"
                rel="noopener noreferrer"
                className="mt-3 inline-flex max-w-full items-center gap-1.5 text-sm font-medium text-[#0A66C2] hover:underline"
              >
                <Linkedin className="h-4 w-4" strokeWidth={2} />
                <span className="truncate">linkedin.com/in/aballeyguier</span>
              </a>
            </div>
          </aside>
        </BlurFade>
        ...
        <BlurFade delay={0.3} duration={0.6}>
          <section className="flex h-full flex-col justify-center rounded-[1.75rem] border border-emerald-600/15 bg-gradient-to-br from-emerald-50/80 via-white to-white p-8 shadow-[0_24px_80px_-56px_rgba(16,185,129,0.55)]">
            ...
            <p className="mt-5 max-w-4xl text-2xl leading-snug text-slate-700">
              André leads Anthropic&apos;s Applied AI practice across EMEA. He previously
              founded and built DataRobot&apos;s EMEA practice and led global AI Solutions
              at Snorkel AI. He has 12 years of experience in applied AI/ML for the
              enterprise.
            </p>

            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <Stat label="Background" value="DataRobot · Snorkel AI" />
              <Stat label="Role" value="Applied AI Leader, EMEA" />
              <Stat label="Education" value="CentraleSupélec" />
            </div>
          </section>
        </BlurFade>
      </div>
